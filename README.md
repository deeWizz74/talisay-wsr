...
contact. An admin account (no public sign-up) can edit names, upload photos, and
set Messenger/phone info for anyone, live, from the browser.
...
Then open http://localhost:3000. On first run, a random admin password is
printed once in the terminal — copy it before it scrolls away. Log in at
`/admin/login` and use **Change password** to set your own.

## Data

Everyone's info lives in `data/directory.json`. It was seeded from the original
Excel org charts (`scripts/seed.js`) and is now edited entirely through the admin
UI — don't re-run `npm run seed` unless you want to wipe admin edits back to the
original Excel data.
...
admin password), so it needs somewhere that keeps `server.js` running and
persists that state — not a purely serverless host. Render's free tier works
and needs no credit card, with one caveat: **redeploying the app resets its
disk** back to whatever was last committed to GitHub, wiping any photos or
edits made through the admin UI since then. Two things guard against that:
...
Render gives you a URL like `https://talisay-wsr-directory.onrender.com` —
that's the link to share with the stake. The free tier sleeps after 15
minutes of inactivity, so the first visit after a quiet stretch takes
~30–60 seconds to wake up; after that it's fast again.
